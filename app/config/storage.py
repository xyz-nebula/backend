...
from app.config.config import VALKEY_HOST, VALKEY_PORT, VALKEY_DB
...
class ValkeyConfig(StorageConfig):
    ...
    host: str = Field(default_factory=VALKEY_HOST)
    port: int = Field(default_factory=VALKEY_PORT)
    db: int = Field(default_factory=VALKEY_DB)
    ...
    def __repr__(self) -> str:
        ...
        Examples:
            >>> config = ValkeyConfig(host="redis-server", port=6380, db=1)
            >>> repr(config)
            'ValkeyConfig(storage_type=redis, host=redis-server, port=6380, db=1, url=redis://redis-server:6380/1)'
        """
    ...
    def get_url(self) -> str:
        ...
        Examples:
            >>> config = ValkeyConfig(host="redis-server", port=6380, db=1, password="secret")
            >>> config.get_url()
            'redis://:secret@redis-server:6380/1'

            >>> config_no_pass = ValkeyConfig(host="redis-server", port=6380, db=1)
